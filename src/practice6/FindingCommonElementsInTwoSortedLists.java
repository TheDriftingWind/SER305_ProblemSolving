...
public class FindingCommonElementsInTwoSortedLists {
	...
	public static void main(String[] args) {
		...
		while(pointer_i <= list_m.length-1 && pointer_c <= list_n.length-1){
			...
			if(Integer.parseInt(list_m[pointer_i]) == Integer.parseInt(list_n[pointer_c])){
				...
				while(pointer_i < list_m.length && Integer.parseInt(list_m[pointer_i]) == last_num){
					pointer_i++; //find the next new number
				}
				if(pointer_i < list_m.length){
